...
async function loginVet() {
    ...
    const request = await fetch('/api/loginVet', {
      method: 'POST',
      headers: {
    ...
    if (request.status == 200) {
      ...
      if (payload.role === 'VET') {
          window.location.href = "/consultationVet.html"; // Redirige a la página de PetOwner
      } else {
          alert("Acceso denegado. No tienes permisos para acceder a esta página.");
          window.location.href = "/loginVet.html"; // O cualquier página que elijas
      }
  } else {
